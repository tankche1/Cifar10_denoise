------------------------------------------------------------------------
--[[ ReinforceSample ]]-- 
-- This implements a policy gradient algorithm
...
-- gradOutputs are ignored (policy gradient algorithm).

local ReinforceSample, parent = torch.class("nn.ReinforceSample", "nn.Reinforce")

function ReinforceSample:updateOutput(input)
	-- print(input)
	if input:dim() == 1 then input = input:view(1, unpack(input:size():totable())) end
...
function ReinforceSample:updateGradInput(input, gradOutput)
	-- Note that gradOutput is ignored
	-- f: action probability mass funciton
	...
	-- add a entropy term to encourage explaration
	local entropyBata = 0.01
	self.gradInput:add(entropyBata, torch.log(self._input)+1)

	return self.gradInput
end

function ReinforceSample:type(type, tc)
   self._index = nil
